...
namespace RentACar.Business.ValidationRules.FluentValidation
{
    public class AppUserAddValidator : AbstractValidator<AppUserAddDto>
    {
        ...
        {
            RuleFor(x => x.FirstName).NotEmpty().WithMessage("First name cannot be empty").MinimumLength(2).WithMessage("First name must be at least 2 characters").MaximumLength(20).WithMessage("First name must be shorter than 20 characters");
            ...
            RuleFor(x => x.UserName).NotEmpty().WithMessage("User name cannot be empty").MinimumLength(2).WithMessage("User name must be at least 2 characters").MaximumLength(20).WithMessage("User name must be shorter than 20 characters");
